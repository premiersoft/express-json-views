...
JsonViewEngine.prototype.render = function (filePath, data, settings, useCache) {
	...
	return this.loadView(filePath, useCache)
		.then(function (viewObj) {
			return Promise.resolve(data).then(function(dataObj){
				return self.renderData(viewObj, dataObj, settings, useCache)
			})
		});
};
...
JsonViewEngine.prototype.renderObject = function (viewObj, data, settings, useCache) {
	...
	var rendered = _.mapValues(viewObj, function (options, attr) {
		...
		if (options.format) {
			value = self.callFormat(self.helpers, options.format, value, data);
		}
...
JsonViewEngine.prototype.callFormat = function(helpers, functionName, value, data) {
	var self = this;
	var args = functionName.split(".");
	if (args.length > 1) {
		var firstArg = args.shift();
		var formatHelper = helpers[firstArg];
		if (formatHelper && _.isObject(formatHelper)) {
			var rest = args.join(".");
			return self.callFormat(formatHelper, rest, value, data);
		}
	}

	var formatFunction = helpers[functionName];
	if (formatFunction && _.isFunction(formatFunction)) {
		return formatFunction.call(helpers, value, data);
	}
};
